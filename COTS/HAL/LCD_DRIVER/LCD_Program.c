...
void HLCD_vSendString(u8 *A_u8String) {
	...
	for (u8 Local_u8Index = 0; A_u8String[Local_u8Index] != '\0';
			Local_u8Index++) {
...
/**
 *  @brief parse integer values to strings to be displayed on the lcd
 * */
void HLCD_vINTtoString(u16 A_u16Integer) {

	u8 local_u8String[16];
	itoa(A_u16Integer, local_u8String, 10);
	HLCD_vSendString(local_u8String);
}
